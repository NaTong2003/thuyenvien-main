...
//lấy id của role thuyền viên theo bảng role
        $users = User::where('role_id', $seafarerId)
// Lấy danh sách các user có vai trò (role_id) bằng id vừa tìm được
                    ->with('thuyenVien.position', 'thuyenVien.shipType')
//lấy thông tin thuyền viên qua bảng chức danh và loại tàu
                    ->paginate(10);
    ...
    public function store(Request $request)
    {
        ...
        // Tìm role 'Thuyền viên' và gán vào biến $seafarerRole.
        // Tạo user mới
        $user = User::create([
            'name' => $request->name,
            'email' => $request->email,
        ...
        if ($user) {//Kiểm tra nếu có user (biến $user tồn tại), thì thực hiện xử lý ti
            ThuyenVien::create([
                'user_id' => $user->id,
    ...
    public function show($id)
    {
       ...
       //User lấy thông tin từ bảng chức danh, loại tàu và test của thuyền viên theo ID.
                    ->findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->filled('password')) {//Kiểm tra xem trường password trong request có được nhập không 
    ...
    public function destroy($id)
    {
        ...
        // Xóa user
        $user->delete();
//Phải xoá thuyền viên trước để đảm bảo không vi phạm ràng buộc khoá ngoại, 
// và để đảm bảo khi xoá user thì không còn dữ liệu liên quan chưa được xử lý.
        return redirect()->route('admin.users.index')
